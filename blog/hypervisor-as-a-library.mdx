...
![catsay](/images/hypervisor-as-a-library/catsay.png)
...
That said, writing a hypervisor is not that hard. The hardest part is to design how you interact with hypervisor, or design the API of the hypervisor. Starina needs a seamless integration with Linux.

In this post, I'd share an interesting pattern I found *"hypervisor as a library"*.
...
impl FileLike for YourVirtualFile {
    fn read_at(
        ...
        // This writes into the virtio ring directly!
...
Starina Linux image is currently embedded into each applicaiton for the time being, but is only 6.7 MB. Also, while it takes 2 seconds in QEMU software emulation to boot the kernel and run `catsay`, I'm pretty sure it can be optimized to less than 100ms (in QEMU), or even less than 10ms with VM snapshotting (again, in QEMU software emulation!).

Optimizing the boot time is not priority as I gotta implement more other features, but I'm very optimistic about its practicality. So the answer is, yes, I think it's a good idea!
