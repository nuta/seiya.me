...
This step is also the first time I needed to inject interrupts into the guest. While RISC-V spec is clean, but I was totally lost with how to do it. Setting `hideleg` was what I needed to do, but I was confused by RISC-V Advanced Interrupt Architecture, which extends the RISC-V interrupt handling. RISC-V extensions are more somtimes look like "patches" to the base spec. [RISC-V Unified Database](https://github.com/riscv-software-src/riscv-unified-db) is a great resource to understand how the extensions extend the base spec.
