...
Back in 2000s, CGI (Common Gateway Interface) is how people used to provide dynamic contents on Web. Each time a request comes in, the server spawns a new process to let the program do its job, and generates a response. We don't use CGI today and instead we run applications as long-running processes for resource efficiency and performance.
...
My ultimate goal is to run Starina directly on Raspbery Pi 5, with good observability and good automated deployment mechanism similar to Kubernetes.
